...
def normalize_audio(audio_array, sample_rate):
    ...
    audio_resampled = librosa.resample(audio_array, orig_sr=sample_rate, target_sr=TARGET_SAMPLE_RATE)
    ...
    for root, dirs, files in os.walk(input_dir):
        for file in files:
            if file.endswith(".txt") and file.startswith("transcription"):
                ...
                output_file_path = transcription_file_path.replace(input_dir, output_dir)
    ...
    for root, dirs, files in os.walk(input_dir):
        for file in files:
            if file.endswith(".wav"):
                ...
                audio_array, sample_rate = librosa.load(file_path, sr=None)  # Load with original sample rate
                ...
                save_processed_audio(audio_normalized, TARGET_SAMPLE_RATE, processed_audio_path)
                ...
                spectrogram = extract_log_mel_spectrogram(audio_normalized, TARGET_SAMPLE_RATE)
                spectrogram_file_path = processed_audio_path.replace(".wav", "_log_mel_spectrogram.npy")
